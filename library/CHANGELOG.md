...
Bug修复：
1. 修复多条件匹配删除时只有第一个条件生效的问题；

优化：
1. 新增多条件的删除、查询、更新测试用例覆盖；
2. 清除2.0版本之前的冗余代码和测试用例，压缩Har包体积；

## 2.0.2

优化：
1. 支持状态管理V2装饰器@ObservedV2与@Trace和ORM内装饰器一起使用；

注意：
1. 由于装饰器Observed会清空类名属性，导致模型丢失类名，所以ibest-orm不支持与状态管理V1装饰器一起使用；

## 2.0.1

新增：
1. 更新关联关系的使用文档和测试用例；
2. 修复级联操作ManyToMany无法自动创建中间表问题；
